...
getGEOSuppFiles("GSE20966", baseDir = download_dir)
untar("GSE20966_RAW.tar", exdir = "C:/Users/adity/Documents/RScripts/GSE20966")

proc_data <- getGEO('GSE20966')
...
list.files("C:/Users/adity/Documents/RScripts/GSE20966", full.names = TRUE)
cel_files <- list.files("C:/Users/adity/Documents/RScripts/GSE20966", pattern = "*.CEL.gz", full.names = TRUE)
...
disease_state <- phenotype_data[["disease:ch1"]]
...
pca_pre_plot <- ggplot(pca_celdata_df, aes(x = PC1, y = PC2, color = DiseaseState)) +
  ...
  scale_color_manual(values = c("type 2 diabetes" = "red", "non-diabetic control" = "blue")) +
  theme_minimal()
pca_pre_plot

rma_norm_data <- rma(celdata, background = TRUE, normalize= TRUE, subset= NULL )
phenotype_rma <- pData(rma_norm_data)
exprs_rma <- exprs(rma_norm_data)
write.csv(phenotype_rma, file = "phenotype_rma.csv", row.names = TRUE)

boxplot_rma <- boxplot((exprs_rma), main = "Normalized Data", outline = FALSE)

pca_norm <- prcomp(t(exprs_rma), scale. = TRUE)
disease_state_n <- phenotype_rma[['disease:ch1']]
...
Probeset_gene_id <- biomaRt::getBM(attributes = c("hgnc_symbol", "ensembl_gene_id",
                                                  "affy_u133_x3p"),
                                   filters = "affy_u133_x3p",
                                   values = probeIDs,
                                   mart = mart)

match_rows <- match(rownames(exprs_rma), Probeset_gene_id$affy_u133_x3p)     
...
write.csv(exprs_anno, file = "annotated_expression_data.csv", row.names = TRUE)
...
design
contrast_matrix <- makeContrasts(T2DvsControl = type.2.diabetes - non.diabetic.control,
                                 levels = design)
